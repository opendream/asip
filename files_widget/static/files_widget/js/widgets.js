$(function(){
    ...
    function fillInHiddenInputs(dropbox, movedOutFile, movedInFile) {
        console.log(dropbox);
        var scope = dropbox.parent('.files-widget').filter(':visible');

        var input = $('input[name="' + dropbox.data('input-name') + '_0"]', scope),
            deletedInput = $('input[name="' + dropbox.data('input-name') + '_1"]', scope),
            movedInput = $('input[name="' + dropbox.data('input-name') + '_2"]', scope),
    ...
    widget.each(function() {
        var that = $(this),
            ...
            hiddenInput = $('input[name="' + dropbox.data('input-name') + '_0"]', that),
        ...
        dropbox.on('click', '.enlarge-button', function() {
            var image_path = $(this).closest('.preview').data('image-path');
            if (image_path.search('http') == 0) {
                window.open($(this).closest('.preview').data('image-path'));
            }
            else {
        ...
        var options = {
            ...
            add: function(e, data) {
                ...
                if (filesInput.hasClass('file-widget-image-wg')) {
                    acceptFileTypes = /(\.|\/)(gif|jpe?g|png)$/i;
                }
                else {
                    acceptFileTypes = /^[^(\.|\/)]+$|(\.|\/)(?!(ade|adp|bat|chm|cmd|com|cpl|exe|hta|ins|isp|jar|jse|lib|lnk|mde|msc|msp|mst|pif|scr|sct|shb|sys|vb|vbe|vbs|vxd|wsc|wsf|wsh)$)([^.]+$)/;
                }
                ...
                if(data.originalFiles[0]['size'].length && data.originalFiles[0]['size'] > 5000000) {
                    uploadErrors.push('File size is too large');
                }
        ...
        if (filesInput.hasClass('file-widget-image-wg')) {
            ...
            options['acceptFileTypes'] = /^[^(\.|\/)]+$|(\.|\/)(?!(ade|adp|bat|chm|cmd|com|cpl|exe|hta|ins|isp|jar|jse|lib|lnk|mde|msc|msp|mst|pif|scr|sct|shb|sys|vb|vbe|vbs|vxd|wsc|wsf|wsh)$)([^.]+$)/
        }

        filesInput.fileupload(options);
    });
});
