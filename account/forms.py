...
from account.models import AppConnect, AbstractPeopleField
...
import files_widget
from common.forms import PermalinkForm, CommonModelForm, EnglishCharField, PrettyCheckboxSelectMultiple
...
from taxonomy.models import Country, UserRole, Topic, JobRole, Location
from tagging.forms import TagField
from tagging_autocomplete_tagit.widgets import TagAutocompleteTagIt


class EmailAuthenticationForm(forms.Form):
    email = EnglishCharField()
...
class InviteForm(PasswordResetForm):
    ...
    first_name = EnglishCharField(required=False, max_length=255, widget=forms.TextInput())
    last_name = EnglishCharField(required=False, max_length=255, widget=forms.TextInput())
    ...
    summary = EnglishCharField(required=False, max_length=255, widget=forms.TextInput())
...
class AccountEditForm(PermalinkForm):
    ...
    username    = EnglishCharField(max_length=30)
    ...
    first_name  = EnglishCharField(required=True, max_length=30, widget=forms.TextInput())
    last_name   = EnglishCharField(required=True, max_length=30, widget=forms.TextInput())
    skills = TagField(required=False, widget=TagAutocompleteTagIt(max_tags=False), help_text='')


    occupation  = EnglishCharField(required=False, max_length=128, widget=forms.TextInput())
    ...
    homepage_url = EnglishCharField(required=False, max_length=255, widget=forms.TextInput())
    ...
    notification_allow_email_send_organization_organizationparticipate = forms.NullBooleanField(required=False, widget=forms.CheckboxInput(attrs={'id': 'notification_allow_email_send_organization_organizationparticipate'}))
    ...
    job_status = forms.ChoiceField(required=False, widget=forms.RadioSelect, choices=AbstractPeopleField.FULL_JOB_STATUS_CHOICES)
    job_public_status = forms.ChoiceField(required=False, widget=forms.RadioSelect, choices=[
        (True, '<span class="choice-item-wrapper"><span class="choice-text">Yes, display my profile</span></span>'),
        (False, '<span class="choice-item-wrapper"><span class="choice-text">No, hide my profile</span></span>')
    ])

    job_position = forms.ChoiceField(required=False, widget=forms.RadioSelect, choices=AbstractPeopleField.FULL_JOB_POSITION_CHOICES)

    money_salary = MoneyField(required=False, max_digits=19, decimal_places=2)
    # job_primary_role = TreeNodeChoiceField(required=False, queryset=JobRole.objects.all())
    ...
    job_locations = forms.ModelMultipleChoiceField(required=False, queryset=Location.objects.all(), widget=PrettyCheckboxSelectMultiple())
    job_criteria = EnglishCharField(required=False, widget=forms.Textarea(attrs={'rows':5, 'maxlength': 300}), max_length=300)

    job_email = forms.EmailField(required=False, label=_("Contact Email"), max_length=255)
    job_telephone = forms.CharField(required=False, label=_("Contact Telephone"), max_length=255)
    ...
    def __init__(self, inst=None, model=None, request_user=None, required_password=False, required_job=False, *args, **kwargs):
        ...
        if required_job:
            self.fields['attachments'].required = True
            self.fields['job_status'].required = True
            self.fields['job_public_status'].required = True
            self.fields['job_position'].required = True
            self.fields['money_salary'].required = True
            self.fields['job_roles'].required = True
            self.fields['job_locations'].required = True
            self.fields['job_criteria'].required = True
...
class AccountRegisterForm(PasswordResetForm):
    ...
    def clean_email(self):
        ...
        try:
            UserModel.objects.get(email__iexact=email, is_active=True)
            raise forms.ValidationError(_('This email is already in use.'))
        except UserModel.DoesNotExist:
